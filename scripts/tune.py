...
parser.add_argument("--cuda_id", required=True,
                    help="which CUDA")
...
def main(args):
    if args.algo == 'ppo':
        sweep_config = {
            "name": "_".join([args.env, args.algo]),
            "method": "random",
        ...
        "parameters": {
            ...
            "num_frames_per_proc": dict(values=[5,10,20]),
            'gae_lambda':{"min": 0.9, "max": 0.99},
    ...
    wandb_train_func = functools.partial(train, args, seeds, device)
    wandb.agent(sweep_id, function=wandb_train_func, count=args.count)

def train(args,seeds,devices):
    project_name = "_".join(["tuning",args.env,args.algo])
        ...
        for seed in seeds:
                ...
                if args.algo == "a2c":
                    hypes={
                            ...
                            "lr":config["lr"],
                            "gae_lambda":config['gae_lambda'], # surrogate loss term
                            "entropy_coef": config["entropy-coef"], # entropy loss term
                            "max_grad_norm":args.max_grad_norm,
                            'value_loss_coef':args.value_loss_coef,
                            "recurrence" : config["recurrence"],
                            "rmsprop_eps" :args.optim_eps,
                            "rmsprop_alpha" :args.optim_alpha}
                ...
                if args.algo == "ppo":
                    hypes={
                            ...
                            "lr":config["lr"],
                            "gae_lambda":config['gae_lambda'], # surrogate loss term
                            "clip_eps" :config["clip-eps"], # surrogate loss term
                            "entropy_coef": config["entropy-coef"], # entropy loss term
                            # "value_loss_coef": config["value-loss-coef"], # value loss term
                            "max_grad_norm":args.max_grad_norm,
                            'value_loss_coef':args.value_loss_coef,
                            "recurrence" : config["recurrence"],
                ...
                while num_frames < args.frames:
                    ...
                    if update % args.log_interval == 0:
                        ...
                        mean_reward=np.mean(list(rreturn_per_episode.values()))
                        print(mean_reward)
                        wandb.log({"ep_rew_mean": mean_reward})
